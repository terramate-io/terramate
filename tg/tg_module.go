...
func LoadModule(rootdir string, dir project.Path, fname string, trackDependencies bool) (mod *Module, isRootModule bool, err error) {
	...
	// Create a Terragrunt-compatible logger
	tgLogger := NewTerragruntLogger(logger)
	// Close the logger to prevent goroutine and file descriptor leaks
	defer func() {
		if adapter, ok := tgLogger.(*zeroLogAdapter); ok {
