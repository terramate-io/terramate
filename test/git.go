...
func NewGitWrapper(t testing.TB, wd os.Path, env []string) *git.Git {
	t.Helper()
...
func EmptyRepo(t testing.TB, bare bool) os.Path {
	t.Helper()
...
func NewRepo(t testing.TB) os.Path {
	t.Helper()
	...
	err := gw.RemoteAdd("origin", remoteDir.String())
	assert.NoError(t, err, "git remote add origin")

	path := WriteFile(t, repoDir, "README.md", "# generated by terramate tests")
	assert.NoError(t, gw.Add(path.String()), "adding README.md to remote repo")
