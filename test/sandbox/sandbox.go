...
func NoGit(t testing.TB) S {
	...
	buildTree(t, &config.Root{
		Tree: config.NewTree(outerDir),
	}, []string{
...
func (s S) Generate() generate.Report {
	return s.GenerateAt(s.Config().Tree, s.RootDir())
}
...
func (s S) LoadStacks() stack.List {
	s.t.Helper()
	return s.LoadStacksFromRoot(s.ReloadConfig())
}

// LoadStacksFromRoot load all stacks from the provided root config.
func (s S) LoadStacksFromRoot(root *config.Root) stack.List {
	s.t.Helper()

	entries, err := terramate.ListStacks(root.Tree)
	assert.NoError(s.t, err)
...
func (s S) LoadStackGlobals(
	...
	ctx, report := stack.LoadStackGlobals(root, projmeta, sm)
...
func (de DirEntry) ListGenFiles(cfg *config.Root) []string {
	de.t.Helper()
...
func (se StackEntry) Load(cfg *config.Root) *stack.S {
	se.t.Helper()
...
func buildTree(t testing.TB, tree *config.Root, layout []string) {
	t.Helper()
