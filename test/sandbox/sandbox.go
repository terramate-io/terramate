...
// GenerateWith is like Generate but uses the cfg as config.
func (s S) GenerateWith(cfg *config.Tree) generate.Report {
	t := s.t
	t.Helper()

	report := generate.Do(cfg)
...
func (s *S) ReloadConfig() *config.Tree {
	s.t.Helper()
	s.cfg = nil
	return s.Config()
}
...
func (de DirEntry) ListGenFiles(cfg *config.Tree) []string {
	de.t.Helper()
