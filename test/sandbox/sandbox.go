...
const (
	...
	DefaultGitTemplate TemplateKind = "default_git"
)

var (
	templateMtx   sync.Mutex
	templateCache map[TemplateKind]string
)

func init() {
	templateCache = make(map[TemplateKind]string, 0)
}

const sandboxDir = "sandbox"

func lookupTemplate(t testing.TB, k TemplateKind) string {
	templateMtx.Lock()
	defer templateMtx.Unlock()
	...
	case DefaultTemplate:
		dir = createDefaultTemplate(t)

	case DefaultGitTemplate:
		dir = createDefaultGitTemplate(t)

	default:
		t.Fatalf("unknown template kind: %v", k)
	}
...
func createDefaultTemplate(t testing.TB) string {
	t.Helper()
	...
	outerdir, err := os.MkdirTemp("", "terramate-test-templates")
	...
	rootdir := filepath.Join(outerdir, sandboxDir)
	test.MkdirAll(t, rootdir)

	return outerdir
}

func createDefaultGitTemplate(t testing.TB) string {
	t.Helper()

	outerdir := createDefaultTemplate(t)
	rootdir := filepath.Join(outerdir, sandboxDir)

	remotedir := filepath.Join(outerdir, sandboxDir+"-remote")
	test.MkdirAll(t, remotedir)
	...
	path := test.WriteFile(t, rootdir, "README.md",
		fmt.Sprintf("# generated by terramate (entropy %d)", time.Now().UnixNano()))
	git.Add(path)
	git.Commit("first commit")

	test.WriteFile(t, rootdir, ".gitignore", defaultGitIgnoreContent)
	git.CommitAll("add gitignore")

	baregit := test.NewGitWrapper(t, remotedir, []string{})
	assert.NoError(t, baregit.Init(remotedir, "main", true))

	git.RemoteAdd("origin", "../sandbox-remote")
	git.PushOn("origin", "main", "main")
...
// New creates a new complete test sandbox that is initialized from a template.
// The git repository is set up with sane defaults.
//
// It is a programming error to use a test env created with a testing.TB other
// than the one of the test using the test env, for a new test/sub-test always
// create a new test env for it.
func NewFromTemplate(t testing.TB, k TemplateKind) S {
	t.Helper()

	srcdir := lookupTemplate(t, k)
	dstdir := test.TempDir(t)

	assert.NoError(t, fs.CopyAll(dstdir, srcdir))
	rootdir := filepath.Join(dstdir, sandboxDir)
