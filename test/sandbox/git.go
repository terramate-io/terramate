...
func NewGit(t testing.TB, repodir os.Path) *Git {
	t.Helper()
...
// repository.
func (git *Git) BareRepoAbsPath() os.Path {
	git.t.Helper()
	if git.bareRepo == "" {
...
func (git *Git) configureDefaultRemote() {
	...
	git.RemoteAdd(cfg.DefaultRemoteName, remoteRepo.String())
...
func (git Git) SetupRemote(remoteName, remoteBranch, localBranch string) {
	...
	git.RemoteAdd(remoteName, remoteRepo.String())
	git.PushOn(remoteName, remoteBranch, localBranch)
}

func (git *Git) initRemoteRepo(branchName string) os.Path {
	t := git.t
	t.Helper()
...
func (git Git) Clone(repoURL string, dir os.Path) {
	git.t.Helper()
