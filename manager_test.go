...
func TestListChangedStackReason(t *testing.T) {
	...
	assert.EqualStrings(t, "/", changed[0].Stack.Path().String(), "stack dir mismatch")
	...
	changed = report.Stacks
	assert.EqualInts(t, 1, len(changed), "unexpected number of entries")
	assert.EqualStrings(t, "/stack", changed[0].Stack.Path().String(), "stack dir mismatch")
...
func assertStacks(
	...
	for i := 0; i < len(want); i++ {
		assert.EqualStrings(t, want[i], got[i].Stack.Path().String(), "path mismatch")
...
func singleNotChangedStack(t *testing.T) repository {
	...
	// make it a stack
	assert.NoError(t, stackfs.Create(root, stackfs.CreateCfg{Dir: repo}), "terramate init failed")
	assert.NoError(t, g.Add(stackfs.DefaultFilename), "add terramate file failed")
...
func multipleStacksOneChangedRepo(t *testing.T) repository {
	...
	root, err := config.LoadRoot(repo.Dir)
	assert.NoError(t, err)
	assert.NoError(t, stackfs.Create(root, stackfs.CreateCfg{Dir: otherStack}), "terramate init failed")

	assert.NoError(t, g.Add(filepath.Join(otherStack, stackfs.DefaultFilename)),
...
func singleStackSingleModuleChangedRepo(t *testing.T) repository {
	...
	root, err := config.LoadRoot(repo.Dir)
	assert.NoError(t, err)
	assert.NoError(t, stackfs.Create(root, stackfs.CreateCfg{Dir: st}))

	g := test.NewGitWrapper(t, repo.Dir, []string{})

	test.WriteFile(t, st, "main.tf", `
module "something" {
