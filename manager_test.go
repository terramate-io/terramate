...
func TestListChangedStackReason(t *testing.T) {
	...
	assert.EqualStrings(t, "/", changed[0].Stack.Dir, "stack dir mismatch")
	...
	assert.NoError(t, err, "unexpected error")
	assert.EqualInts(t, 1, len(changed), "unexpected number of entries")
	assert.EqualStrings(t, "/stack", changed[0].Stack.Dir, "stack dir mismatch")
...
func assertStacks(
	...
	for i := 0; i < len(want); i++ {
		assert.EqualStrings(t, want[i], got[i].Stack.Dir, "path mismatch")
...
func multipleStacksOneChangedRepo(t *testing.T) repository {
	...
	assert.NoError(t, terramate.Init(repo.Dir, otherStack, false), "terramate init failed")
