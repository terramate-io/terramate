...
func TestApplyDependencyFilters(t *testing.T) {
	...
	type testcase struct {
		name           string
		layout         []string
		filterOpts     engine.OutputsSharingOptions
...
func TestDeprecatedFlags(t *testing.T) {
	...
	result, err := e.ApplyDependencyFilters(engine.OutputsSharingOptions{
		IncludeOutputDependencies: true,
	}, initialSelection, "")
	assert.NoError(t, err)

	// Should include both stack-a and stack-b
	assert.EqualInts(t, 2, len(result))

	resultPaths := make(map[string]bool)
	for _, st := range result {
		resultPaths[st.Stack.Dir.String()] = true
	}
	assert.IsTrue(t, resultPaths["/stack-a"])
