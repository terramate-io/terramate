...
func (e *Engine) NewDependencyGraph(stacks config.List[*config.SortableStack], tgModules tg.Modules, target string) (*DependencyGraph, error) {
	...
	if tgModules != nil {
		tgModuleMap := make(map[string]*tg.Module)
		for _, mod := range tgModules {
			tgModuleMap[mod.Path.String()] = mod
		}

		for _, st := range stacks {
			stackPath := st.Stack.Dir.String()
			tgMod, hasTgModule := tgModuleMap[stackPath]
			if !hasTgModule {
				continue
			}

			// Scan for dependency blocks in the Terragrunt module
			// The tg.Module struct already processed these during LoadModule
			// They're stored in the After field, which we already processed above
			// But we need to also handle the dependency blocks themselves
			// Let's reload to get the dependency blocks
			trackDeps := true
			reloadedMod, _, err := tg.LoadModule(rootcfg.HostDir(), st.Stack.Dir, "terragrunt.hcl", trackDeps)
			if err != nil {
				logger.Debug().
					Err(err).
					Str("stack", stackPath).
			...
			// The After field contains paths from dependencies.paths
			// We've already processed those above from stack.After
			// For dependency blocks, they should also be in After based on the LoadModule code
			_ = reloadedMod
			_ = tgMod
		}
	}
...
// GetDependenciesForStacks returns all direct dependencies for a set of stacks
func (g *DependencyGraph) GetDependenciesForStacks(stackPaths []string) []string {
