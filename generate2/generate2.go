...
func Load(root *config.Root, vendorDir project.Path) ([]LoadResult, error) {
	...
	for i, st := range stacks {
		...
		generated, err := loadStackCodeCfgs(root, st.Stack, ctx, vendorDir, nil)
		if err != nil {
...
func doStackGeneration(
	...
	generated, err := loadStackCodeCfgs(root, stack, ctx, vendorDir, vendorRequests)
	if err != nil {
		report.err = err
		return report
	}
...
func stackOutdated(
	...
	ctx := eval.NewContext(stdlib.Functions(st.HostDir(root)))
	generated, err := loadStackCodeCfgs(root, st, ctx, vendorDir, nil)
	if err != nil {
		return nil, err
	}
