...
func TestTmJoinListCustomErrorTypes(t *testing.T) {
	...
	t.Run("JoinListInvalidArgumentTypeError", func(t *testing.T) {
		...
		var argErr stdlib.JoinListInvalidArgumentTypeError
		assert.IsTrue(t, errors.As(err, &argErr))
		assert.EqualStrings(t, "string", argErr.Got)
	})
	...
	t.Run("JoinListInvalidElementTypeError", func(t *testing.T) {
		...
		var elemErr stdlib.JoinListInvalidElementTypeError
		assert.IsTrue(t, errors.As(err, &elemErr))
		assert.EqualInts(t, 0, int(elemErr.Index))
		assert.EqualStrings(t, "number", elemErr.Got)
	})
	...
	t.Run("JoinListInvalidListElementTypeError", func(t *testing.T) {
		...
		var listErr stdlib.JoinListInvalidListElementTypeError
		assert.IsTrue(t, errors.As(err, &listErr))
		assert.EqualInts(t, 0, int(listErr.Index))
		assert.EqualStrings(t, "number", listErr.Got)
	})
	...
	t.Run("JoinListInvalidTupleElementTypeError", func(t *testing.T) {
		...
		var tupleErr stdlib.JoinListInvalidTupleElementTypeError
		assert.IsTrue(t, errors.As(err, &tupleErr))
		assert.EqualInts(t, 0, int(tupleErr.ListIndex))
		assert.EqualInts(t, 1, tupleErr.ElementIndex)
		assert.EqualStrings(t, "number", tupleErr.Got)
	})
}
