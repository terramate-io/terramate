...
func TestTmTreeComprehensiveErrors(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "cycle detection with proper tree",
			input:    `tm_tree([[null, "root"], ["root", "A"], ["A", "B"], ["B", "A"]])`,
			errorMsg: []string{"multiple parents", "A"}, // A has root and B as parents - conflicting parents
		},
		{
...
func TestTmTreeErrorTypes(t *testing.T) {
	// Test that IsTreeError works correctly with direct error values
	t.Run("error type checking", func(t *testing.T) {
		err1 := stdlib.TreeErrorInvalidPairLength()
		assert.IsTrue(t, stdlib.IsTreeError(err1, stdlib.TreeErrorInvalidInput))
		assert.IsTrue(t, !stdlib.IsTreeError(err1, stdlib.TreeErrorCycle))

		err2 := stdlib.TreeErrorCycleNode("test")
		assert.IsTrue(t, stdlib.IsTreeError(err2, stdlib.TreeErrorCycle))
		assert.IsTrue(t, !stdlib.IsTreeError(err2, stdlib.TreeErrorInvalidInput))

		// Test with non-TreeError
		normalErr := errors.New("some other error")
		assert.IsTrue(t, !stdlib.IsTreeError(normalErr, stdlib.TreeErrorCycle))
	})
	...
	t.Run("error messages", func(t *testing.T) {
		...
		err = stdlib.NewTreeError(stdlib.TreeErrorCycle, "custom message: %s %d", "test", 42)
