...
func (s *Spec) Exec(ctx context.Context) error {
	if s.AllTerraform || s.EnsureStackIDs || s.AllTerragrunt {
...
func (s *Spec) execScanCreate(ctx context.Context) error {
	...
	if scanFlags == 0 {
		return errors.E("path argument or one of --all-terraform, --all-terragrunt, --ensure-stack-ids must be provided")
	}

	if scanFlags > 1 {
		return errors.E("only one of --all-terraform, --all-terragrunt, --ensure-stack-ids can be provided")
	}
	...
	case "--all-terragrunt":
		return s.initTerragrunt(ctx)
	case "--ensure-stack-ids":
		return s.ensureStackID(ctx)
	}
	panic("unexpected case")
}

func (s *Spec) initTerragrunt(ctx context.Context) error {
	rootdir := s.Engine.Config().HostDir()
...
func (s *Spec) initTerraform(ctx context.Context) error {
	err := s.initTerraformDir(ctx, s.WorkingDir)
	if err != nil {
...
func (s *Spec) initTerraformDir(ctx context.Context, baseDir string) error {
	rootdir := s.Engine.Config().HostDir()
	...
	for _, f := range dirs {
		...
		if f.IsDir() {
			errs.Append(s.initTerraformDir(ctx, path))
			continue
		}
...
func (c *Spec) ensureStackID(ctx context.Context) error {
	report, err := c.Engine.ListStacks(engine.NoGitFilter(), cloudstack.AnyTarget, cloud.NoStatusFilters(), false)
	if err != nil {
	...
	for _, entry := range report.Stacks {
		...
		c.Printers.Stdout.Println(fmt.Sprintf("Generated ID %s for stack %s", id, entry.Stack.Dir))
	}
	return nil
}
