...
func Do(rootdir string, workingDir string) Report {
	report, generatedFiles := forEachStack(rootdir, workingDir, func(
...
outdatedDirsLoop:
	for _, outdatedDir := range outdatedDirs {
		...
		for _, outdatedFile := range outdatedDir.files {
			outdatedAbsPath := filepath.Join(outdatedDir.dir, outdatedFile)
			// Stacks can generated code outside them, so we need to check if
			// the file is on a non-stack dir + it is not generated by a stack.
			if generatedFiles.has(outdatedAbsPath) {
				continue
			}
			if err := os.Remove(outdatedAbsPath); err != nil {
