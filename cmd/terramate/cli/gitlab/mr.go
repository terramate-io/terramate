...
// LatestMRsForCommit returns a list with latest Merge Requests associated with the provided commit SHA.
func (c *Client) LatestMRsForCommit(ctx context.Context, sha string, limit int) (mrs MRs, err error) {
	var url string
	if c.ProjectID != 0 {
		url = fmt.Sprintf("%s/projects/%d/repository/commits/%s/merge_requests?per_page=%d", c.baseURL(), c.ProjectID, sha, limit)
	} else {
		url = fmt.Sprintf("%s/projects/%s/repository/commits/%s/merge_requests?per_page=%d", c.baseURL(), c.projectRef(), sha, limit)
	}
	...
	if err != nil {
		return nil, errors.E(err, "unmarshaling MR list")
	}
	return mrs, nil
}
