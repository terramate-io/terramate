...
func TestGitlabSmokeTestMR(t *testing.T) {
	...
	mrs, err := client.LatestMRsForCommit(ctx, "2b2383915aa09510b9ef77a48617dff8c2ad17ad", 1)
	assert.NoError(t, err)
	assert.EqualInts(t, 1, len(mrs), "expected a single MR but got %d: payload: %v", len(mrs), mrs)
	mr := mrs[0]
	expected := gitlab.MR{
		ID:           297900704,
		IID:          1,
