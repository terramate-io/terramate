...
type (
	...
	Manager struct {
		rootcfg    *config.Root // whole config
...
// NewManager creates a new stack manager. The rootcfg is the project's
// root configuration and gitBaseRef is the git reference to compare against for
// changes.
func NewManager(rootcfg *config.Root, gitBaseRef string) *Manager {
	return &Manager{
		rootcfg:    rootcfg,
...
func (m *Manager) List() (*StacksReport, error) {
	...
	entries, err := ListStacks(m.rootcfg.Tree)
	if err != nil {
		return nil, err
	}
	...
	logger.Trace().Str("repo", m.rootcfg.RootDir()).
		Msg("Create git wrapper for repo.")

	g, err := git.WithConfig(git.Config{
		WorkingDir: m.rootcfg.RootDir(),
	})
	if err != nil {
...
func (m *Manager) ListChanged() (*StacksReport, error) {
	...
	if !g.IsRepository() {
		return nil, errors.E(errListChanged, "the path \"%s\" is not a git repository", m.rootcfg.RootDir())
	}
	...
	changedFiles, err := listChangedFiles(m.rootcfg.RootDir(), m.gitBaseRef)
	if err != nil {
		return nil, errors.E(errListChanged, err)
	}
	...
	for _, path := range changedFiles {
		...
		dirname := filepath.Dir(filepath.Join(m.rootcfg.RootDir(), path))

		if _, ok := stackSet[project.PrjAbsPath(m.rootcfg.RootDir(), dirname)]; ok {
			continue
		}
		...
		cfgpath := project.PrjAbsPath(m.rootcfg.RootDir(), dirname)
		stackTree, found := m.rootcfg.Lookup(cfgpath)
		if !found || !stackTree.IsStack() {
			...
			for checkdir.String() != "/" {
				...
				stackTree, found = m.rootcfg.Lookup(checkdir)
				if found && stackTree.IsStack() {
					break
				}
			}
	...
	allstacks, err := ListStacks(m.rootcfg.Tree)
	if err != nil {
...
func (m *Manager) AddWantedOf(scopeStacks stack.List) (stack.List, error) {
	...
	wantsDag := dag.New()
	allstacks, err := stack.LoadAll(m.rootcfg.Tree)
	if err != nil {
