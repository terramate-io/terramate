...
func BuildDAG(
	...
	ancestorStacks, err := stack.StacksFromTrees(root.Dir(), root.Tree().StacksByPaths(s.Path(), ancestorPaths...))
	if err != nil {
		return errors.E(err, "stack %q: failed to load the \"%s\" stacks",
			s, ancestorsName)
	}
	descendantStacks, err := stack.StacksFromTrees(root.Dir(), root.Tree().StacksByPaths(s.Path(), descendantPaths...))
	if err != nil {
		return errors.E(err, "stack %q: failed to load the \"%s\" stacks",
			s, descendantsName)
	}
